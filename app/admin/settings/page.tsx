    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-neutral-border shadow-xs">
        ...
        <div className="flex items-center space-x-2">
          <span
            className={`text-xs font-bold uppercase px-3 py-1 rounded-md font-mono ${
              isAdmin
      ...
      {!isAdmin && (
        <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl text-amber-800 text-xs flex items-start space-x-3">
      ...
      {savedSuccess && (
        <div className="p-4 bg-green-50 border border-green-200 rounded-xl text-green-700 text-xs flex items-center space-x-2 animate-in fade-in">
      ...
      {loading ? (
        <div className="p-12 text-center bg-white rounded-2xl border border-neutral-border">
          ...
          <p className="text-xs text-text-dark/60">Loading settings...</p>
        </div>
      ) : (
        <form onSubmit={handleSave} className="space-y-6">
          {/* Section 1: NAP Contact Details */}
          <div className="bg-white p-6 sm:p-8 rounded-2xl border border-neutral-border shadow-xs space-y-5">
            <h3 className="text-sm font-bold uppercase tracking-wider text-primary border-b border-neutral-border pb-3 flex items-center space-x-2">
              <Phone className="w-4 h-4 text-[#1E5FA8]" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl border border-neutral-border text-xs focus:outline-none focus:border-[#1E5FA8] disabled:bg-neutral-light"
                />
              </div>
          ...
          <div className="bg-white p-6 sm:p-8 rounded-2xl border border-neutral-border shadow-xs space-y-5">
            <h3 className="text-sm font-bold uppercase tracking-wider text-primary border-b border-neutral-border pb-3 flex items-center space-x-2">
              <Globe className="w-4 h-4 text-[#1E5FA8]" />
          ...
          {isAdmin && (
            <div className="flex justify-end">
              <button
                type="submit"
                disabled={saving}
                className="inline-flex items-center space-x-2 bg-[#1E5FA8] hover:bg-[#2C74C9] text-white font-bold px-6 py-3 rounded-xl text-xs shadow-xs transition-colors disabled:opacity-60"
              >
                {saving ? (
                  <>
